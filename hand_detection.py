...
#setting up the screen size and the sensitivity
def __init__(img_shape):
    mouse = PyMouse()
    shape_x, shape_y, _ = img_shape
    screen_x, screen_y = mouse_cntrl.screen_size()
    x_convert = screen_x * 1.0 / shape_x 
    y_convert = screen_y * 1.0 / shape_y
    sensitivity = 1.0

def screen_size():
    return mouse.screen_size()

#perform mouse relese
def release():
    x, y = mouse.position()
    mouse.release(x,y)

#performs right click of mouse
def rightclick():
    current_x, current_y = mouse.position()
    mouse.click(current_x, current_y, right_click)


#performs leftclick of mouse
def leftclick():
    current_x, current_y = mouse.position()
    mouse.click(current_x, current_y, left_click)
...
def gestures():
    ...
    cv2.setMouseCallback('frame',mousePosition,param)
    while True:
        ...
        #finding the largest contours area in the image
        max_area = -1
